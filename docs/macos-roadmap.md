...
| Diagram Editor | Dimensões, título, marcas, pestanas, rótulos, estilos, preview e clipboard | PARCIAL — criar/editar, título, cordas, casas, preview, salvar e cancelar reais; fechamento protege alterações não salvas com Salvar/Não salvar/Cancelar |
...
| Element editors | Mark, Barre, Fret Label e Style | PENDENTE |
...
- As issues `#8` a `#13` formaram o conjunto inicial de bloqueadores conhecidos
  da versão estável para macOS; a `#13` foi resolvida pelo fluxo de proteção de
  alterações não salvas no editor.
